...
export default function ConfirmPaymentPin() {
  ...
  const paymentIntentIdParam = paramToString(paymentIntentId);
  ...
  const [autoVerificationAttempted, setAutoVerificationAttempted] =
    useState(false);
  ...
  const paymentTypeValue = isValidPaymentType(paymentTypeParam)
    ? paymentTypeParam
    : undefined;
  const requiresOtp = paymentTypeValue === "wallet";
  const requiresPaymentIntent = paymentTypeValue === "stripe";
  ...
  const handleVerifyPayment = useCallback(
    async (otp?: string) => {
      ...
      if (requiresPaymentIntent && !paymentIntentIdParam) {
        showNewToast({
          title: "Missing Stripe Details",
          description:
            "Unable to verify your Stripe payment because paymentIntentId is missing.",
          icon: HelpCircleIcon,
          action: "error",
          variant: "solid",
        });
      ...
      try {
        ...
        if (requiresPaymentIntent && paymentIntentIdParam) {
          payload.paymentIntentId = paymentIntentIdParam;
        }
  ...
  useEffect(() => {
    if (
      ...
      (!requiresPaymentIntent || !!paymentIntentIdParam)
    ) {
      ...
      >
        <ThemedView className="flex-1">
          <ThemedView className=" justify-center items-center mt-20">
            <ThemedText type="default" className="text-typography-500">
              {requiresOtp
                ...
                : "We\u2019re confirming your payment. This could take a moment."}
            </ThemedText>
          </ThemedView>
          ...
          {requiresOtp ? (
            ...
            <ThemedView className="mt-12 items-center px-6">
              {loading && <ActivityIndicator size="large" color="#111827" />}
              <ThemedText
                type="default"
                className="text-typography-600 text-center mt-4"
              >
      ...
      {showModal && (
        <>
          <CustomModal
            ...
            firstBtnLink={"/(tabs)/notifications"}
            firstBtnText="Go to Notifications"
            secondBtnLink={"/(tabs)"}
            secondBtnText="Home"
